    ...
    async loadData() {
        try {
            const header = await this.orm.call(
                'cashflow.report','get_header',[this.state.bankSelection],
            );
            const headerRed = await this.orm.call(
                'cashflow.report','index_header_red',[this.state.bankSelection]
            );
            this.cashflowDashboard.headerRed = headerRed;
            console.log(headerRed);
            const headerGreen = await this.orm.call(
                'cashflow.report','index_header_green',[this.state.bankSelection]
            );
            this.cashflowDashboard.headerGreen = headerGreen;
            this.cashflowDashboard.header = header;
            const data = await this.orm.call(
                'cashflow.report','get_values',[this.state.bankSelection],
            );
            //console.log(data);
            this.cashflowDashboard.data = data;
            /**}
            else{
                alert('La période choisi est superieure a 15 jours')}**/
